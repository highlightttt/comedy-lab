// Comedy Lab Feedback System v3
// Saves to localStorage + sends to local server + posts to Discord via webhook
(function() {
  ...
  function syncToServer(d) {
    // Try local server (silent fail on mobile/remote)
    fetch(LOCAL_SERVER, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({page: PAGE, feedback: d})
    }).catch(() => {});
  }
  ...
  function createModal() {
    ...
    style.textContent = `
      ...
      .fb-modal-saved.fail{color:#ff6b35}
    `;
  ...
  window._fbSubmitModal = function(comment) {
    ...
    // Write to feedback file on server
    const box = document.querySelector('.fb-modal-box');
    
    // Send individual vote to feedback endpoint
    fetch(LOCAL_SERVER + '/vote', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      ...
      if (r.ok) {
        ...
        box.innerHTML = `<div class="fb-modal-saved ok">✅ Saved locally</div>`;
      }
      setTimeout(() => window._fbCloseModal(), 800);
    }).catch(() => {
      // Server not available — that's fine, saved to localStorage
      box.innerHTML = `<div class="fb-modal-saved ok">✅ Saved locally</div>`;
      setTimeout(() => window._fbCloseModal(), 800);
    });
  ...
  window.vote = function(btn) {
    ...
    setTimeout(() => {
      const input = modal.querySelector('.fb-modal-input');
